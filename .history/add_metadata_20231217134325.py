...
def meta_control(path):
    print(path)
    source_file = Path.joinpath(Path.cwd(), 'metadata')
    target_dir = Path(path)
    # 复制文件
    for dof in source_file.iterdir():
        # dof.copy(target_dir)
        print(dof)
        # copyfile(dof, str(target_dir))
        shutil.copy(Path.joinpath(source_file, dof), Path.joinpath(source_file, dof)target_dir)
    # if source_file.is_file():
    #     (target_dir/source_file.name).write_text(source_file.read_text(encoding="utf-8"), encoding="utf-8")
    # else:
